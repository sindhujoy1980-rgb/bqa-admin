...
export default function Sidebar() {
  ...
  return (
    <aside style={{
        ...
        {open ? (
          <Link href="/overview" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flex: 1, minWidth: 0 }}>
            ...
            <div style={{ overflow: 'hidden' }}>
              <div style={{ fontWeight: 800, fontSize: 13.5, color: 'var(--text)', whiteSpace: 'nowrap', lineHeight: 1.2 }}>Bible Quiz Daily</div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', whiteSpace: 'nowrap', lineHeight: 1.3 }}>Admin Portal</div>
            </div>
          </Link>
        ) : (
        ...
        {NAV.map((sec, si) => (
          <div key={si} style={{ marginBottom: 4 }}>
            {open ? (
              ...
              si > 0 && <div style={{ height: 1, background: 'var(--border)', margin: '6px 6px 8px' }} />
            )}
              ...
              return (
                <Link key={item.path} href={item.path} title={!open ? item.name : undefined}
                  style={{
                    ...
                    background: isActive
                      ? 'linear-gradient(90deg, rgba(37,99,235,0.16), rgba(37,99,235,0.04))'
                      : 'transparent',
                    borderLeft: isActive && open ? '2px solid var(--primary)' : '2px solid transparent',
                    transition: 'all 0.15s ease',
                    overflow: 'hidden', whiteSpace: 'nowrap',
                  }}>
                  <Icon size={16} strokeWidth={isActive ? 2 : 1.6}
                    style={{ flexShrink: 0, color: isActive ? 'var(--primary-light)' : 'inherit', transition: 'color 0.15s' }} />
                  {open && <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</span>}
                  {open && isActive && (
                    <div style={{
                      width: 5, height: 5, borderRadius: '50%', flexShrink: 0,
                      background: 'var(--primary)', boxShadow: '0 0 8px rgba(37,99,235,0.9)',
                    }} />
                  )}
                </Link>
              );
      ...
      {/* Theme toggle */}
      <div style={{ padding: open ? '8px 10px' : '8px', borderTop: '1px solid var(--border)', zIndex: 1 }}>
        <button onClick={toggle} title={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
          style={{
            ...
            cursor: 'pointer', color: 'var(--text-sub)', fontFamily: 'inherit',
            fontSize: 12.5, fontWeight: 500, transition: 'all 0.15s ease',
          }}>
          {theme === 'dark'
            ? <Sun size={15} strokeWidth={1.8} style={{ flexShrink: 0, color: '#f59e0b' }} />
            : <Moon size={15} strokeWidth={1.8} style={{ flexShrink: 0, color: '#60a5fa' }} />}
          {open && (theme === 'dark' ? 'Light Mode' : 'Dark Mode')}
        </button>
      </div>

      {/* Account */}
      <div style={{ borderTop: '1px solid var(--border)', flexShrink: 0, position: 'relative', zIndex: 1 }}>
        {showAccount && open && (
          <div style={{
            ...
            boxShadow: '0 -12px 48px rgba(0,0,0,0.4)', zIndex: 60,
          }}>
            {admin && (
              <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)' }}>
                ...
                <div style={{ fontWeight: 700, color: 'var(--text)', fontSize: 13.5 }}>{admin.name}</div>
            ...
            <div style={{ padding: 6 }}>
              <Link href="/profile" onClick={() => setShowAcc(false)}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', gap: 9,
                  padding: '8px 10px', borderRadius: 8, background: 'transparent',
                  fontSize: 12.5, color: 'var(--text-sub)', textDecoration: 'none',
